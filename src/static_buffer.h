 ...
 *                 and, on success, 0
 */
#define BUF_SET_MIN_SIZE(TYPE, SIZE, INI_FUNC) \
    do { \
 ...
 */
#define BUF_GET_NEXT_REF(TYPE, INC, REF, INI_FUNC) \
    do { \
        /* Check if the buffer must be expanded */ \
        if (_##TYPE##_buf.used >= _##TYPE##_buf.len) { \
            BUF_SET_MIN_SIZE(TYPE, _##TYPE##_buf.len + INC, INI_FUNC); \
        } \
